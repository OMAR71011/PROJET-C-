...
void ajouter_etudiant(Etudiant etudiants[], int *nb_etudiants) {
    ...
    while (continuer) {
        ...
        printf("\nEtudiant ajoute et sauvegarde dans le fichier CSV !\n");
...
void sauvegarder_etudiants(Etudiant etudiants[], int nb_etudiants) {
    ...
    fprintf(fichier, "numero,code_classe,nom,prenom,email,date_naissance\n");
    for (int i = 0; i < nb_etudiants; i++) {
        fprintf(fichier, "%d,%04d,%s,%s,%s,%s\n",
                etudiants[i].numero,
...
int demander_continuer(const char *message) {
    ...
    while (1) {
        ...
        printf("Réponse invalide. Tapez oui/non ou o/n.\n");
    }
}
